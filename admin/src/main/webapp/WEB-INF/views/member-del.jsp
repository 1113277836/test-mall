	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach items="${requestScope.members}" var="member">
			<tr class="text-c">
				...
				<td>${member.id}</td>
				<td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.jsp','10001','360','400')">${member.username}</u></td>
				<td>${member.tel}</td>
				<td>${member.idNumber}</td>
				<td class="text-l">北京市 海淀区</td>
				<td class="td-status"><span class="label label-danger radius">${member.status}</span></td>
				<c:if test="${fn:endsWith(member.status,'已删除' )==true}">
				<td class="td-manage">
					<a style="text-decoration:none" name="renew"  href="javascript:;" title="还原"><i class="Hui-iconfont">&#xe631;</i></a>
					</c:if>
					<c:if test="${fn:endsWith(member.status,'已恢复')==true}">
...
<script type="text/javascript">
    $(function () {
        $("a[name=renew]").click(function () {
            var id= $(this).next("input[name='id']").val();
            console.log(id);
            //点击按钮，发起ajax请求
            $.ajax({
                //请求地址
                url: "${ctx}/member/renewMember",
                //请求类型
                type: "POST",
                //请求参数
                data: {
